...
```{r}
geneTrain <- measurementsVS[, 1:135]
geneTest <- measurementsVS[, 126:235]
clinicalTrain <- sampleInfo[1:135, ]
clinicalTest <- sampleInfo[126:235, ]
classesTrain <- classes[1:135]
classesTest <- classes[126:235]
```
...
We could also use logistic regression with the gene expression data.


```{r}

X = as.data.frame(t(geneTrain))
y = clinicalTrain$Response
newX = as.data.frame(t(geneTest))

# Perform logistic regression 
LRclassifierGene <- glm(y ~ ., data = X,family =  "binomial")

# Predict classes by probability > 0.5
LRprobabilitiesGene <- predict(LRclassifierGene, newX , type = "response")
LRclassesGene <- ifelse(LRprobabilities > 0.5, levels(classes)[2], levels(classes)[1])

# Calculate confusion matrix. 
LRconfusionGene <- table(actual = classesTest, predicted = LRclassesGene)
LRconfusionGene

# Calculate accuracy as the number of predictions the model got correct

sum(diag(LRconfusionGene))/sum(LRconfusionGene)
...
# Perform logistic regression 
LRclassifierGeneBest10 <- glm(y ~ ., data = X,family =  "binomial")
LRprobabilitiesGeneBest10 <- predict(LRclassifierGeneBest10, newX , type = "response")

# Predict classes by probs > 0.5
LRclassesGeneBest10 <- ifelse(LRprobabilitiesGeneBest10 > 0.5, levels(classes)[2], levels(classes)[1])

# Calculate confusion matrix. 
LRconfusionGeneBest10 <- table(actual = classesTest, predicted = LRclassesGeneBest10)
LRconfusionGeneBest10

# Calculate accuracy as the number of predictions the model got correct

sum(diag(LRconfusionGeneBest10))/sum(LRconfusionGeneBest10)

# Which genes appear to be contributing the most to the model?

summary(LRclassifierGeneBest10)
...
An implementation of DLDA is provided by the R package sparsediscrim. The `dlda` function trains the classifier and the `predict` function is used to make predictions on test data. It requires that the features be the columns of the table. We will again use the top 10 most differentially expressed genes.
